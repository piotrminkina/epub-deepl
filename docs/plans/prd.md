...
|---|---|---|---|
| SM-1 | Round-trip integrity (no translation) | 100% of test corpus | Composite check: (a) `diff -r` of unzipped EPUBs shows no content differences for every EPUB in the corpus (bundled: one book per supported EPUB version; extendable via `EPUB_DEEPL_CORPUS`); (b) output ZIP has `mimetype` as first entry, STORED, `flag_bits=0`, no extra-field bytes; (c) all other entries DEFLATED; (d) output passes `zipfile.ZipFile.testzip()` |
...
| SM-4 | EPUB validity | Output preserves the `epubcheck` baseline | Run `epubcheck` on every corpus book after round-trip-without-translation; no new errors vs the input's own baseline (US-016) |
